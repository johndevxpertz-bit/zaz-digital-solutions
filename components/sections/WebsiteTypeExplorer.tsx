...
type WebsiteTypeExplorerProps = {
  types: TypeTab[];
  /** Pre-rendered (server-side) panel content keyed by `${buildType}__${typeSlug}`. */
...
export default function WebsiteTypeExplorer({ types, panels }: WebsiteTypeExplorerProps) {
  ...
  return (
    <div>
      ...
      <div key={`${buildType}__${activeType}`} className="zaz-tile-enter mt-10">
        {panels[`${buildType}__${activeType}`]}
      </div>
    </div>
  );
}
